...
const MemoryGame = (() => {
  ...
  function create(seed=1) { const g={version:'M0.3',rng:seed>>>0,round:1,starter:0,turn:0,acted:0,phase:'turn',step:'aux',pending:null,failed:false,events:[],players:[],seats:GODS.map(()=>({state:'active',weather:0,offerings:[]}))};
  ...
  function choose(g,who,policy='balanced') {
    ...
    function value(a){const s=g.seats[a.seat],own=s?s.offerings.filter(o=>o.player===who).length:0;
      switch(a.type){
        ...
        case 'rest':return !saved&&active===1?-100:25+(g.round>=5?12:0)-(own>=2?25:0)-(!saved&&s.offerings.length===3?25:0);
  ...
  function story(g){
    return GODS.map((god,id)=>{
      ...
      const chosen=[...firstByPlayer,sacrifice,struggle,call,weather,fate,after].filter(Boolean);
      ...
      const turns=[sacrifice,struggle].filter(Boolean).sort((a,b)=>a.seq-b.seq).map(e=>e.text);
  ...
  return {E,GODS,STORMS,create,legal,act,actor,score,choose,chooseClassic,check,story,epilogue,cardName};
})();
