...
function restHint(who){const q=g.pending,canKeep=M.legal(g,who).some(a=>a.type==='keep');return `${g.players[q.attacker].name}已付两张记忆与主行动。${g.players[who].name}是其他贡献者中供奉最多的一位，由其唯一回应（同数按安魂者之后的顺序）。`+(who===0?'你可永久燃掉一张匹配记忆或神明记忆，留下1伤痕（−1分），阻止安魂并降1风化，供奉不动；也可让祂安息。':'等待其选择留名或送别。')+(who===0&&!canKeep?(M.living(g,who)<=6?'存续记忆已到六张底线，不能再燃。':'手中没有匹配记忆，此刻只能送别。'):'')+(!M.hasTimeToName(g,q.seat)?'剩余主行动已不足以填满并呼名；留名不会增加行动。':'')+'无论回应如何，都回到安魂者整理，不额外获得主行动。';}
...
function apply(id){try{const eventStart=g.events.length;const who=M.actor(g);g=M.act(g,who,id);M.check(g);MemorySession.record(seed,who,id);selected=null;render();playRitual(g.events.slice(eventStart));}catch(e){$('hint').textContent='火塘暂歇：'+e.message;clearTimeout(timer);}}
function render(){clearTimeout(timer);const ended=g.phase==='ended',who=M.actor(g),p=g.players[0];
 ...
 $('gods').replaceChildren(...g.seats.map((s,i)=>{const d=M.GODS[i],n=node('article',undefined,'god '+(s.state==='active'?'':s.state==='awake'?'awake':'retired'));n.style.setProperty('--fade',s.state==='ruin'?1:s.state==='awake'?0:s.weather/3);const img=node('img');img.src='./art/'+d.image;img.alt=d.name;const body=node('div',undefined,'body');body.append(node('h2',s.state==='ruin'?'□□□':d.name),node('p',({active:`风化 ${s.weather}/3${s.offerings.length===3?' · 真名齐备，等待呼名':''}`,awake:'名字重新完整',rest:'已送别',ruin:'已被遗忘'})[s.state]));const slots=node('div',undefined,'slots');for(const e of d.elements){const o=s.offerings.find(o=>o.element===e),slot=node('div',e,'slot');slot.append(node('span',o?g.players[o.player].name:s.state==='active'?'空缺':'已归还'));if(o)slot.style.setProperty('--ink',colors[o.player]);slots.append(slot);}if(d.title)body.append(node('p',d.title));body.append(slots,node('p',d.effect));n.append(img,body);return n;}));
...
$('auto').onclick=()=>{spectate=!spectate;render();};$('restart').onclick=()=>{if(!confirm('熄灭当前实验火塘，重新开始？'))return;clearRitual();g=M.create(++seed);MemorySession.reset(seed);filter='';selected=null;spectate=false;render();};render();
...
function playRitual(events){
 ...
 // 同次风化毁掉多座时，每座都有局部反馈；字幕只保留最重要的一次。
