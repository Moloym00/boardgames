...
function render(){clearTimeout(timer);const ended=g.phase==='ended',who=M.actor(g),p=g.players[0];
 ...
 const turning=g.events.filter(e=>['burn','awake','communal','rest','ruin','displace','defend'].includes(e.type)).at(-1);
 ...
 $('actions').replaceChildren(...visible.map(a=>{const b=node('button',a.label+(a.type==='awaken'?' · '+awakeningPreview(a.seat):a.type==='rest'&&!g.seats.some(s=>s.state==='awake')&&g.seats.filter(s=>s.state==='active').length===1?' · 最后一尊神：安魂后将共同失败':''));b.onclick=()=>{if(a.type==='burn'&&!confirm('这张记忆将永远离开，你留下1点伤痕（−1分）。仍要燃忆吗？'))return;apply(a.id);};return b;}));
 ...
 $('ending').replaceChildren();if(ended){
   ...
   const keep=node('button','保存这一夜的故事');keep.onclick=()=>{
     const text=['真名之火 · M0.3 · 这一夜',...g.players.map(p=>p.name+'：'+M.score(p)+'分'),g.failed?'共同失败：没有神被唤醒。':'至少一个名字被留下。',...M.story(g).flatMap(c=>[c.title,c.narrative,...c.moments.map(e=>`第${e.round}更 · ${e.text}`)]),...losses].join('\n');
...
$('auto').onclick=()=>{spectate=!spectate;render();};$('restart').onclick=()=>{if(!confirm('熄灭当前实验火塘，重新开始？'))return;g=M.create(++seed);MemorySession.reset(seed);filter='';selected=null;spectate=false;render();};render();
